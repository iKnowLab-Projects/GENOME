...
from .utils import ensure_gender_proportions, RowList, first_non_null_datetime, process_column_names, convert_df_columns_from_field_ids_to_field_names
...
class Evaluator:
    ...
    def _load_aux_data(self):
        ...
        #     .loc[:, Col.GENDER]\
        #     .dropna()
        ukb_genders_df = self.dst.ukb.read_processed(self.dst.ukb.to_schema([Fld.GENDER]))
        ukb_genders_df = process_column_names(ukb_genders_df)
        ukb_genders_df = convert_df_columns_from_field_ids_to_field_names(ukb_genders_df, self._dd)
        self.ukb_genders = ukb_genders_df.loc[:, Col.GENDER].dropna()
            ...
            for name, schema in self._schema.items():
                if name == UKB:
                    ...
                    df = self.dst[name].load(schema, rows=rows)\
                        .concat().compute()
                if df.empty:
                    logging.warning(f"No data found for source {name}, skipping...")
                    continue
    ...
    def _select_estimator(self):
        ...
        if X0.empty:
            logging.warning("Empty training data, skipping estimator selection")
            return
    ...
    def _fit_all_models(self):
        X0, y0 = self.Xy(0)
        if X0.empty:
            logging.warning("Empty training data, skipping model fitting")
            return
