...
class UkbPatientSelector:
    ...
    SAMPLING_FACTORS = Col.all_values()
    ...
    def _stratified_sample(self, cases: pd.Index, controls: pd.Index):
        # Filter cases and controls to only include IDs that exist in sampling_factors
        valid_cases = cases.intersection(self.sampling_factors.index)
        valid_controls = controls.intersection(self.sampling_factors.index)
        
        # Check if we have any valid cases to work with
        if len(valid_cases) == 0:
            # If no valid cases, just return a simple Series with all cases and controls
            return pd.concat([
                pd.Series(True, cases, name='is_case'), 
                pd.Series(False, controls, name='is_case')])
        
        result = stratified_sample(
            self.sampling_factors.loc[valid_controls].assign(is_case=False),  
            self.sampling_factors.loc[valid_cases].assign(is_case=True),
            self.sampling_factors.columns.to_list(),
            size_factor=self.ukb_controls_cases_ratio)
            
        # Add back any cases and controls that weren't in sampling_factors
        missing_cases = cases.difference(valid_cases)
        missing_controls = controls.difference(valid_controls)
        
        if len(missing_cases) > 0 or len(missing_controls) > 0:
            missing_data = pd.concat([
                pd.Series(True, missing_cases, name='is_case'),
                pd.Series(False, missing_controls, name='is_case')
            ])
            # Combine with the stratified sample result
            result = pd.concat([result['is_case'], missing_data])
        else:
            result = result['is_case']
    ...
    def __call__(self, cohort_spec, 
        ...
        if self.infrequent_gender_thresh and self.sampling_factors is not None:
            # remove gender whose frequency is under the threshold
            # First, get only the matched_ids that actually exist in sampling_factors
            valid_ids = matched_ids.intersection(self.sampling_factors.index)
            if len(valid_ids) > 0:
                genders = self.sampling_factors.loc[valid_ids, Col.GENDER]
                freq = genders.value_counts(normalize=True)
                if freq.min() < self.infrequent_gender_thresh:
                    discarded_gender = freq.index[freq.argmin()]
                    valid_ids_to_keep = genders.index[genders != discarded_gender]
                    # Keep only the valid IDs that meet gender criteria plus the IDs we didn't check
                    matched_ids = valid_ids_to_keep.union(matched_ids.difference(valid_ids))
