...
def process_column_names(df):
    ...
    """
    new_columns = df.columns.map(extract_field_id)
    df.columns = new_columns
    return df


def convert_df_columns_from_field_ids_to_field_names(df, dd):
    """Convert the column names of a pandas DataFrame from field IDs to field names.
    """
    new_columns = df.columns.map(lambda x: dd.find(field_id=int(x)).title.values[0])
    df.columns = new_columns
    return df


def convert_df_dtypes_to_categorical(df):
    """Convert the dtypes of a pandas DataFrame to categorical.
    """
