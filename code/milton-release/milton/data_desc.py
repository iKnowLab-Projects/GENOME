...
class Col:
    ...
    @classmethod
    def all_values(cls):
        """Returns a list of all column values defined in this class."""
        all_attrs = vars(cls)
        values = []
        
        for k, v in all_attrs.items():
            if not k.startswith('_'):  # 언더스코어로 시작하지 않는 속성만
...
class UkbDataDict:
    ...
    def _load_data_field_desc(location, ukb_dir):
        ...
        for c in columns:
            ...
            if '-' in c:
                field_id, version = c.split('-')
            else:
                field_id, version = c.split('.')[0], '.'.join(c.split('.')[1:])
            colmap[field_id].append(version)
